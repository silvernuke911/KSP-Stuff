...
dt = 0.005
max_t = p_1.orbital_period + p_1.orbital_period
...
for i, dep_time in enumerate(departure_time):
    for j, trv_time in enumerate(travel_time):
        p_1state = orbital_elements_to_state_vector(p_1.semi_major_axis,p_1.eccentricity,p_1.inclination,np.radians(p_1.LAN),np.radians(p_1.argument_of_periapsis),np.radians(p_2.mean_anomaly_at_epoch),1,0,dep_time)
        p_2state = orbital_elements_to_state_vector(p_2.semi_major_axis,p_2.eccentricity,p_2.inclination,np.radians(p_2.LAN),np.radians(p_2.argument_of_periapsis),np.radians(p_2.mean_anomaly_at_epoch),1,0,trv_time+dep_time)
        r_p1 = p_1state[0]
        r_p2 = p_2state[0]
        v_p1 = p_1state[1]
        v_p2 = p_2state[1]
...
p_1state = orbital_elements_to_state_vector(p_1.semi_major_axis,p_1.eccentricity,np.radians(p_1.inclination),np.radians(p_1.LAN),np.radians(p_1.argument_of_periapsis),0,1,0,t_i)
p_2state = orbital_elements_to_state_vector(p_2.semi_major_axis,p_2.eccentricity,np.radians(p_2.inclination),np.radians(p_2.LAN),np.radians(p_2.argument_of_periapsis),0,1,0,t_f)
...
plt.plot(rt[:,0],rt[:,1],marker='',zorder=1,color='green')
...
plt.scatter(rt[0][0],rt[0][1],marker='.',s=60,color='blue',zorder=2)
plt.quiver(rt[0][0], rt[0][1], vt[0][0], vt[0][1], angles='xy', scale_units='xy', scale=1, color='g')
...
plt.scatter(r1[t_stepi][0],r1[t_stepi][1],marker='.',s=60,color='blue',zorder=2)
plt.quiver(r1[t_stepi][0], r1[t_stepi][1], v1[t_stepi][0], v1[t_stepi][1], angles='xy', scale_units='xy', scale=1, color='b')

plt.scatter(r2[t_stepf][0],r2[t_stepf][1],marker='.',s=60,color='red',zorder=2)
plt.quiver(r2[t_stepf][0], r2[t_stepf][1], v2[t_stepf][0], v2[t_stepf][1], angles='xy', scale_units='xy', scale=1, color='r')
